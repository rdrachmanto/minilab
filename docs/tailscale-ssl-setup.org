...
1. A TLD from providers such as porkbun, cloudflare, etc.
2. Get account API_KEY, API_SECRET_KEY and enable API access in the domain-level
3. Packages installed: =tailscale=, =dnsmasq=, =caddy=
